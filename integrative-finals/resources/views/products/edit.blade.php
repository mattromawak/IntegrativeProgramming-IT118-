...
@section('content')
    <main class="container">
        <button class="btn btn-danger" onclick="window.location.href='{{ route('products.index') }}'">
            Cancel
        </button>
        <section>
                    ...
                    <div>
                        ...
                        <select name="category" id="">
                            @foreach (json_decode('{"Smartphone":"Smartphone","Smart TV":"Smart TV","Computer":"Computer"}', true) as $optionKey => $optionValue)
                                <option
                                    value="{{ $optionKey }}"{{ isset($product->category) && $product->category == $optionKey ? 'selected' : '' }}>
                                    {{ $optionValue }}</option>
                            @endforeach
                        </select>
                        <hr>
